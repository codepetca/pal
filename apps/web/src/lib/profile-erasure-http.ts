...
import { canonicalUuid, erasureEnabled, validErasureRequest } from "./profile-erasure-contract";
...
export async function profileErasureHttp(req: NextRequest, operationId?: string) {
  try {
    ...
    if (req.nextUrl.search || (req.method === "GET" &&
    ...
    if (req.method === "POST" && !validErasureRequest(body)) {
      return respond({ error: "invalid_erasure_request" }, 422);
    }
    ...
    if (req.method === "GET") {
      const receipt = await getProfileErasure(getDb(), integration.id, operationId!);
      return receipt ? respond(receipt, 200) : respond({ error: "erasure_operation_not_found" }, 404);
    }
    if (!validErasureRequest(body)) return respond({ error: "invalid_erasure_request" }, 422);
    ...
    const receipt = saved.status === "completed" ? saved :
      await progressProfileErasure(getDb(), integration.id, saved.operation_id);
