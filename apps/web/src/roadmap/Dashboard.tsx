...
interface DashboardProps {
  data: RoadmapData;
  /** Controlled editor visibility — opened from the host sandbox control panel. */
  editing?: boolean;
  onEditingChange?: (editing: boolean) => void;
  /** Bump to fire the celebration animation (host "Run animation" control). */
  runSignal?: number;
}
 ...
 */
export function Dashboard({ data: initial, editing = false, onEditingChange, runSignal = 0 }: DashboardProps) {
  ...
  // Host "Run animation" control bumps runSignal to fire a celebration.
  useEffect(() => {
    if (runSignal > 0) setCelebrate((c) => ({ n: c.n + 1, icon: "star" }));
  }, [runSignal]);
