	...
	INSERT INTO "public"."story_collectible_schedules" (
		...
		CASE
			WHEN "late_terminal_value"
				OR (NOT "terminal_weekend_value" AND "due_at_value" < NEW."created_at")
				THEN NEW."created_at"
			ELSE NULL
		END,
		NEW."created_at"
...
BEGIN
	IF TG_OP = 'INSERT' THEN
		...
		INTO
			...
			"source_metadata", "source_due_at", "source_created_at"
		...
		"source_effective_due_at" := CASE
			WHEN "source_terminal_weekend" THEN greatest(
				"source_due_at", "source_created_at", "source_terminal_open_at"
			)
			ELSE "source_due_at"
		END;
		"source_late_terminal" := "public"."is_late_terminal_story_configuration"(
			"source_metadata", "source_created_at"
		);
		...
		IF "source_event_type" IS DISTINCT FROM 'daily_log_week.configured'
			...
			OR NEW."reconciled_at" IS DISTINCT FROM (CASE
				WHEN "source_late_terminal"
					OR (NOT "source_terminal_weekend" AND "source_due_at" < "source_created_at")
...
SET "reconciled_at" = "schedule"."created_at"
FROM "public"."learner_facts" AS "fact"
WHERE "schedule"."source_fact_id" = "fact"."id"
	AND "schedule"."learner_id" = "fact"."learner_id"
	AND "schedule"."reconciled_at" IS NULL
	AND "public"."is_late_terminal_story_configuration"(
		"fact"."metadata", "fact"."created_at"
	);
--> statement-breakpoint
