...
- **Daily-log XP is paid once per day, on the streak advance.** The base 10 xp and the streak bonus both hang off the derived `STREAK_MILESTONE`, not the raw `daily_log.completed` event. Because the milestone only fires when the streak actually advances, repeated daily-log events on the same day earn the day's reward exactly once — they cannot farm the 10 xp base.
