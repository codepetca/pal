...
- **TitleAward** — one durable identity-title award per learner and stable title ID. Its learner-owned source fact preserves the source occurrence, while its creation time records when PAL granted the title under the learner lock. Snapshots select the latest PAL award, so delayed source delivery cannot reorder an identity retroactively; story titles win a same-action tie.
