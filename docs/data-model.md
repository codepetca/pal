...
- **LearnerFact** — an append-only, privacy-safe, semantically unique fact derived from an accepted event. It prevents the same learner behavior from counting twice even if a producer changes the transport idempotency key; only the learner-consent cascade removes it.
...
- **StoryPlan** — one immutable authoritative term start, versioned story identity, and story-assignment count for a learner's opaque academic term. The story-assignment count may be shorter than the academic term: the default Home story has 16 chapters even in a 17–24-week term. Its normalized chapter assignments must cover every contiguous story ordinal at transaction commit and may bind only to an opaque period owned by that learner. Plan identity, story length, and chapter assignments never change after creation; only an initially unbound period key may be attached once. Chapter IDs are catalog references, never student-authored content.
- **StoryCollectibleSchedule** — a typed, durable due-work row derived from the first complete calendar-bearing weekly configuration fact accepted after the scheduling rollout for one learner and opaque period, provided the learner's pinned story plan has a chapter at that week ordinal. Calendar-less V1 configurations remain valid for Weekly Rhythm, but they are not story-schedule eligible because they provide no authoritative timezone, term dates, or week position; a later calendar-bearing revision enrolls that period prospectively only if the revision is otherwise accepted. A closed calendar-less period cannot be revised merely to add story metadata and remains story-ineligible, including when an older calendar-bearing revision arrives late. Neither schedules nor rewards are historically backfilled. Its indexed `due_at` is the authoritative local boundary used for rollout eligibility. Producer `occurred_at` determines whether the configuration preceded that boundary and any terminal-week effective due adjustment; receipt `created_at` remains ingestion/audit bookkeeping. `reconciled_at` normally consumes queue work after the existing reward ledger proves ownership; when a newly accepted configuration was produced after its own due boundary, the row is created terminally reconciled without a grant so it cannot accumulate as invisible pending work or backfill a collectible. It is not a second ownership source. Identity, due fields, and referenced fact provenance are immutable, and deletion follows the source fact's learner-owned cascade.
- **LearnerRewardGrant** — the append-only durable ownership ledger. A `story_chapter` grant references the exact learner-owned plan assignment and is admitted only while that assignment has a matching pending `StoryCollectibleSchedule`; the database silently skips an ineligible old-writer insert so migration-first rolling deploys do not roll back otherwise valid events. A chapter is granted once after its authoritative local instructional week ends, regardless of Weekly Rhythm. The grant stores no sketch/color finish: the snapshot projects that from the durable weekly achievement, allowing a delayed achievement to upgrade presentation without a second item. A `behavior_title` grant references a stable title ID. Every grant references the same learner's source fact, carries database-generated order, and remains owned after `seen_at` is set. Partial uniqueness makes event/cron retries exact-once without deriving ownership from notices or current economy state.
- **LearnerRewardLoadout** — mutable equipped state for the two usable story-reward slots: `companion` and `wallpaper`. Each learner has at most one grant equipped per slot, and the composite foreign key requires the equipped grant to belong to that learner. Story catalogs classify every current reward as `companion`, `wallpaper`, or `keepsake`; keepsakes intentionally create no loadout row and retain the existing reveal-and-dismiss behavior.
