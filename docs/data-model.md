...
- **TitleAward** — one durable identity-title award per learner and stable title ID. Runtime awards keep a learner-owned source fact and source occurrence; null provenance explicitly identifies a pre-ledger migration whose original earning fact cannot be reconstructed. Its creation time records when PAL granted or migrated the title. Snapshots select the latest PAL award, so delayed source delivery cannot reorder an identity retroactively; story titles win a same-action tie.
