...
- `422` — unknown event type, disallowed metadata field, or a malformed/future-dated `occurred_at` (`future_occurred_at`: more than 24h ahead of the server clock — the streak engine is forward-only and a future day would freeze the learner's streak)
