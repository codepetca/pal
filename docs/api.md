...
- `200 { "status": "processed", "mutations": [...] }` — the fact was durably accepted and any eligible rule mutations were applied. A daily log received before its first weekly configuration is accepted with no mutations and a durable pending marker; that first configuration settles no more than its declared eligible-day count after validating source days, with an exact-once settlement marker. `mutations` is the full list the cascade applied, in order; the dev sandbox renders it.
