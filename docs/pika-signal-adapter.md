...
- `term_token` is a stable opaque term identifier. `term_start_day`, `term_end_day`, `term_timezone`, `term_week_count`, `week_start_day`, and `week_index` are an optional additive calendar group: when one is sent, all seven fields are required. Start/end and week start are inclusive calendar dates in the authoritative IANA `term_timezone`; Pal changes active terms and weeks at local midnight in that zone. `term_week_count` is the course's authoritative 6–24 instructional-week count, `week_index` is the period's 1-based position within that count, and `week_start_day` says when that position opens. Pal does not recalculate these values from delivery time or learner history.
...
| `daily_log_week.configured` | Required: `period_key`, `config_version` (integer >= 1), `period_status` (`open` or `closed`), `eligible_days` (integer 0–5). Optional as one all-or-none group: `term_token`, `term_start_day`, `term_end_day`, `term_timezone` (IANA name), `term_week_count` (integer 6–24), `week_start_day` (local `YYYY-MM-DD` inside the term), `week_index` (integer 1–`term_week_count`). | One configuration revision. The idempotency key includes a stable revision token, not merely the period. Version 1 models a Monday–Friday daily-log week. |
