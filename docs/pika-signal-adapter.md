...
- `term_token` is a stable opaque term identifier. `term_start_day`, `term_end_day`, `term_timezone`, and `week_index` are an optional additive calendar group: when one is sent, all five fields are required. Start/end are inclusive calendar dates in the authoritative IANA `term_timezone`; Pal changes active terms at local midnight in that zone. `week_index` is Pika's authoritative 1–16 roadmap position; Pal does not recalculate it from delivery time or learner history.
...
| `daily_log_week.configured` | Required: `period_key`, `config_version` (integer >= 1), `period_status` (`open` or `closed`), `eligible_days` (integer 0–5). Optional as one all-or-none group: `term_token`, `term_start_day`, `term_end_day`, `term_timezone` (IANA name), `week_index` (integer 1–16). | One configuration revision. The idempotency key includes a stable revision token, not merely the period. Version 1 models a Monday–Friday daily-log week. |
...
Roadmap placement is likewise independent of delivery order. New producers send the optional calendar group on `daily_log_week.configured`; Pal converts the snapshot instant to each term's `term_timezone`, selects the term whose inclusive local-date range contains that day, and places each configured period at its `week_index`. If the learner is between terms, Pal retains the most recently completed term; before the first observed term, it selects the nearest upcoming term. Preconfiguring a future term therefore does not replace an active roadmap, and a term transition occurs at the classroom's authoritative local midnight. A delayed correction for an older term or a later legacy configuration cannot roll the roadmap backward or disable the calendar. This means a learner who first enrols in Week 7 opens on Week 7 rather than having that first observed period relabelled Week 1. Pal rejects a period whose calendar fields change and rejects two period keys claiming the same term/week position.

For compatibility during a midterm rollout, periods without the optional calendar group that fall inside the selected term range derive their temporary position from the stored authoritative period anchor and `term_start_day`; calendar-bearing configurations remain authoritative wherever present. Before any calendar group has been observed, Pal retains the original behavior: it sorts opaque periods by their anchors and maps the first 16 to Weeks 1–16. Pika should add the full calendar group to each later configuration and may backfill current-term configurations over time.
