...
For compatibility during a midterm rollout, the original five-field calendar group remains authoritative and implies 16 weeks; a later seven-field revision may upgrade that same term when it declares 16 and preserves the original fields. Periods without any calendar group that fall inside the selected term range derive their temporary roadmap position from the stored authoritative period anchor and `term_start_day`, but that display fallback does not make them story-schedule eligible. Before any calendar group has been observed, Pal retains the original behavior: it sorts opaque periods by their anchors and maps the first 16 to Weeks 1–16. For guaranteed story scheduling, Pika must emit the complete seven-field form with each new term's initial weekly configurations. It may add the calendar group to an open or otherwise legally revisable current-term period, but a closed calendar-less period cannot be enrolled later.
