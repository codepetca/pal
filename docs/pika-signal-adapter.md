...
- Guaranteed story scheduling has a narrower eligibility rule than the backward-compatible event contract: Pika must include the complete five- or seven-field calendar group on the initial weekly configuration, or on a later revision while that period is still legally revisable. A calendar-less configuration remains valid for Weekly Rhythm, but it creates no story due work. Once such a period is closed, ordinary revisions are rejected and it remains permanently story-ineligible; Pal does not backfill its collectible.
...
| `daily_log_week.configured` | Required: `period_key`, `config_version` (integer >= 1), `period_status` (`open` or `closed`), `eligible_days` (integer 0–5). Optional for Weekly Rhythm compatibility but required for story scheduling: original group `term_token`, `term_start_day`, `term_end_day`, `term_timezone` (IANA name), `week_index` (integer 1–16). Adaptive extension: add both `term_week_count` (integer 6–24) and `week_start_day` (local `YYYY-MM-DD` inside the term), with `week_index` inside that count. | One configuration revision. The idempotency key includes a stable revision token, not merely the period. Version 1 models a Monday–Friday daily-log week. |
...
Roadmap placement is likewise independent of delivery order. New producers send the optional calendar group on `daily_log_week.configured`; Pal converts the snapshot instant to each term's `term_timezone`, selects the term whose inclusive local-date range contains that day, sizes the roadmap to `term_week_count`, and places each configured period at its `week_index`. The current roadmap position is the greatest configured `week_index` whose authoritative `week_start_day` has begun; when Week 1 has not been configured, the term start is its compatibility fallback. Before the first authoritative start, schema version 1 deliberately clamps the visible cursor to a Week 1 compatibility placeholder because already-published widgets require a supplied 1-based `currentWeek`; authoritative advancement begins only when a configured week start is reached. During an instructional break, the latest started week remains current until the next `week_start_day`. If the learner is between terms, Pal retains the most recently completed term; before the first observed term, it selects the nearest upcoming term using the same Week 1 compatibility placeholder. Preconfiguring a future term therefore does not replace an active roadmap, and term/week transitions occur at the classroom's authoritative local midnight. A delayed correction for an older term or a later legacy configuration cannot roll the roadmap backward or disable the calendar. This means a learner who first enrols in Week 7 opens on Week 7 rather than having that first observed period relabelled Week 1. Pal rejects a period whose term calendar fields change, rejects a week outside the declared term length, rejects non-increasing week start dates, and rejects two period keys claiming the same term/week position.

For compatibility during a midterm rollout, the original five-field calendar group remains authoritative, is story-schedule eligible, and implies 16 weeks; a later seven-field revision may upgrade that same term when it declares 16 and preserves the original fields. Periods without any calendar group that fall inside the selected term range derive their temporary roadmap position from the stored authoritative period anchor and `term_start_day`, but that display fallback does not make them story-schedule eligible. Before any calendar group has been observed, Pal retains the original behavior: it sorts opaque periods by their anchors and maps the first 16 to Weeks 1–16. For guaranteed story scheduling, Pika must emit a complete five- or seven-field calendar form with each new term's initial weekly configurations; new adaptive-length terms use the seven-field form. Pika may add the calendar group to an open or otherwise legally revisable current-term period, but a closed calendar-less period cannot be enrolled later, including by a delayed older revision.
